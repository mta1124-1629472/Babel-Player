...
public sealed class PythonSubprocessServiceBaseTests : IDisposable
{
    ...
    private static string RequireAnyWorkingPython()
    {
        var pythonPath = DependencyLocator.FindPython();
        Assert.True(pythonPath is not null, "Python not found in test environment");
        return pythonPath!;
    }
    ...
    {
        var pythonPath = RequireAnyWorkingPython();
