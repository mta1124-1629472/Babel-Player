...
Learning: Even with `Process.Kill` and a cancellation token, `StreamReader.ReadToEndAsync` on a redirected standard error pipe may not reliably honor cancellation in all .NET versions. If a dying python process dumps massive tracebacks (e.g., 50MB of stack frames), `ReadToEndAsync` allocates the entire trace and hangs waiting on a pipe that child handles keep open.
