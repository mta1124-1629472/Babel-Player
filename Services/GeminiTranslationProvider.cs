...
public sealed class GeminiTranslationProvider : ITranslationProvider
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        _log.Info($"[GeminiTranslation] Translating {request.SourceLanguage} -> {request.TargetLanguage} with model '{request.ModelName}'.");
        ...
        _log.Info($"[GeminiTranslation] Complete: {translatedArtifact.Segments?.Count ?? 0} segments.");
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        _log.Info($"[GeminiTranslation] Single-segment regen: {request.SegmentId}");
        ...
        _log.Info($"[GeminiTranslation] Single-segment regen complete: {request.SegmentId}");
        ...
        CancellationToken cancellationToken)
    {
        ...
        var responseContent = await client.GenerateTextAsync(
            model,
            BuildSystemPrompt(),
    ...
    private static void ValidateTranslatedArtifact(TranslationArtifact inputArtifact, TranslationArtifact outputArtifact)
    {
        ...
        if (inputSegments.Count != outputSegments.Count)
        {
            throw new InvalidOperationException(
                $"Gemini translation artifact segment count mismatch: expected {inputSegments.Count}, got {outputSegments.Count}.");
        }
        ...
        for (var index = 0; index < inputSegments.Count; index++)
        {
            ...
            if (!string.Equals(expected.Id, actual.Id, StringComparison.Ordinal))
                throw new InvalidOperationException($"Gemini translation changed segment id at index {index}.");
            if (!string.Equals(expected.Text, actual.Text, StringComparison.Ordinal))
                throw new InvalidOperationException($"Gemini translation changed source text for segment '{expected.Id}'.");
            if (Math.Abs(expected.Start - actual.Start) > 0.0001 || Math.Abs(expected.End - actual.End) > 0.0001)
            {
                throw new InvalidOperationException(
                    $"Gemini translation changed timing for segment '{expected.Id}'.");
            }
        }
    }
